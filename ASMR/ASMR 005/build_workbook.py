...
def build_inventory_bottles(wb):
    ...
    for i, row in enumerate(INV_ROWS):
        ...
        put(ws, f"W{r}", f'=IF($N{r}="","",MAX(0,MIN({",".join(args)})-$E$4))',
            font=F_LABEL_B, fmt=FMT_INT, align="center", border=B_ALL)
        put(ws, f"X{r}",
            f'=IF(W{r}>0,"BUY MORE - raises output",IF(N{r}=$E$4,"CURRENT BOTTLENECK",IF(N{r}<=$E$4+2,"NEXT LIMIT","")))',
            font=F_MUTED, align="center", border=B_ALL)
    ...
    ws.conditional_formatting.add(f"A{first}:Y{last}",
        FormulaRule(formula=[f"$N{first}=$E$4"], fill=PatternFill("solid", start_color=RED_FILL)))
    ws.conditional_formatting.add(f"A{first}:Y{last}",
        FormulaRule(formula=[f"$W{first}>0"], fill=PatternFill("solid", start_color=GOLD_LT)))
