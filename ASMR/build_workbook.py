"""
ASMR Production Suite — workbook generator.

Run:  python build_workbook.py
Out:  ASMR_Production_Suite.xlsx  (regenerated deterministically)
...
OUT = "ASMR_Production_Suite.xlsx"
...
FM_ROWS = [
    ("TOP", "Bergamot Accord", 0.090, False, False, "sparkling citrus opening — photosafe accord"),
    ("TOP", "Petitgrain Paraguay", 0.015, False, False, "green-bitter citrus leaf"),
    ("TOP", "Neroli Accord", 0.010, False, False, "orange-flower lift"),
    ("TOP", "Pink Peppercorn", 0.005, True, False, "rosy-spicy glint"),
    ("TOP", "Floralozone", 0.005, True, False, "clean air, sea-light transparency"),
    ("HEART", "Hedione (Firmenich)", 0.200, False, False, "transparent jasmine — the radiance engine"),
    ("HEART", "Orange Blossom Tunisia Absolute", 0.025, False, False, "true white-flower warmth"),
    ("HEART", "Methyl Alpha Ionone Iso", 0.040, False, False, "powdery orris-violet velvet"),
    ("HEART", "Irone Alpha", 0.010, True, True, "precious orris butter — optional luxury"),
    ("HEART", "Benzyl Salicylate", 0.030, False, False, "soft balsamic blender"),
    ("HEART", "Linalool", 0.010, False, False, "fresh floral smoothing"),
    ("BASE", "Iso E Super", 0.150, False, False, "velvet-wood aura, skin-like halo"),
    ("BASE", "Amberxan (Ambroxan)", 0.080, False, False, "mineral amber radiance — the 'still here' trail"),
    ("BASE", "Cashmeran IFF", 0.025, False, False, "cashmere musk-wood"),
    ("BASE", "Sandal Spicatum Australia", 0.040, False, False, "creamy natural sandalwood EO"),
    ("BASE", "Sandalore (Givaudan)", 0.030, False, False, "smooth sandalwood glow"),
    ("BASE", "Javanol", 0.005, True, True, "ultra-modern sandalwood — optional"),
    ("BASE", "Globalide (Habanolide)", 0.065, False, False, "clean airy musk"),
    ("BASE", "Helvetolide", 0.050, False, False, "fruity-ambrette musk"),
    ("BASE", "Ambrettolide (Givaudan)", 0.025, False, False, "seamless skin musk"),
    ("BASE", "Ethylene Brassylate", 0.035, False, False, "soft powder musk"),
    ("BASE", "Exaltolide Total (Firmenich)", 0.020, False, False, "warm intimate musk"),
    ("BASE", "Cedarwood Virginia", 0.020, False, False, "dry pencil-cedar backbone"),
    ("BASE", "Vetyveryle Acetate", 0.005, False, False, "polished vetiver whisper"),
    ("BASE", "Safraleine", 0.010, True, False, "saffron-leather warmth"),
]
FM_FIRST, FM_LAST = 7, 31          # sheet rows of the formula table
FM_TOTAL_ROW = 33

# (material, pack qty, unit, price SAR, density or None, note, assumed)
INV_ROWS = [
    ("Hedione (Firmenich)", 30, "ml", 23.90, 0.90, "", False),
    ("Iso E Super", 10, "ml", 13.60, 0.90, "", False),
    ("Amberxan (Ambroxan)", 10, "g", 133.75, None, "only amber material in use · iterji price 2026-07", False),
    ("Globalide (Habanolide)", 10, "g", 30.00, None, "", False),
    ("Helvetolide", 10, "g", 55.00, None, "", False),
    ("Ambrettolide (Givaudan)", 10, "ml", 92.07, 0.90, "", False),
    ("Ethylene Brassylate", 10, "ml", 10.11, 0.90, "", False),
    ("Exaltolide Total (Firmenich)", 10, "ml", 27.53, 0.90, "", False),
    ("Cashmeran IFF", 10, "ml", 55.86, 0.90, "", False),
    ("Sandal Spicatum Australia", 10, "g", 179.00, None, "", False),
    ("Sandalore (Givaudan)", 10, "ml", 23.54, 0.90, "", False),
    ("Javanol", 10, "g", 81.00, None, "used as 10% D10", False),
    ("Cedarwood Virginia", 10, "ml", 32.50, 0.90, "iterji 'Virginian Cedarwood' · price 2026-07", False),
    ("Vetyveryle Acetate", 10, "g", 124.00, None, "", False),
    ("Methyl Alpha Ionone Iso", 10, "ml", 17.46, 0.90, "", False),
    ("Irone Alpha", 10, "g", 375.00, None, "used as 10% D10 · iterji 'Alpha Irone' price 2026-07", False),
    ("Benzyl Salicylate", 10, "ml", 11.16, 0.90, "", False),
    ("Linalool", 10, "ml", 16.74, 0.90, "", False),
    ("Neroli Accord", 10, "ml", 54.38, 0.90, "", False),
    ("Orange Blossom Tunisia Absolute", 10, "g", 435.00, None, "most precious material", False),
    ("Petitgrain Paraguay", 10, "ml", 43.75, 0.90, "", False),
    ("Pink Peppercorn", 10, "ml", 141.25, 0.90, "used as 10% D10", False),
    ("Bergamot Accord", 10, "ml", 18.75, 0.90, "photosafe — use on skin", False),
    ("Bergamot Essential Oil", 10, "ml", 25.00, 0.90, "PHOTOTOXIC — fabric only", False),
    ("Floralozone", 10, "g", 28.00, None, "used as 10% D10", False),
    ("Safraleine", 10, "g", 81.00, None, "used as 10% D10", False),
    ("DPG (Dipropylene Glycol)", 500, "g", 50.00, None, "EDIT pack grams to actual", True),
    ("Alcohol 190 proof SDA 40B", 1000, "ml", 150.00, 0.81, "EDIT bottle ml to actual", True),
]
INV_FIRST = 4
INV_LAST = INV_FIRST + len(INV_ROWS) - 1   # 31
NOT_IN_FORMULA = {"Bergamot Essential Oil", "DPG (Dipropylene Glycol)", "Alcohol 190 proof SDA 40B"}

# Supplier + product link, verified against iterji.com 2026-07.
# (supplier label, product URL, on_iterji)  — on_iterji False ⇒ highlighted, needs another source.
IT = "https://iterji.com/perfumery-ingredients"
SUPPLIER = {
    "Hedione (Firmenich)":            ("iterji.com", f"{IT}/aroma-chemicals/hedione-firmenich.html", True),
    "Iso E Super":                    ("iterji.com", f"{IT}/aroma-chemicals/iso-e-super.html", True),
    "Amberxan (Ambroxan)":            ("iterji.com", f"{IT}/aroma-chemicals/ambroxan.html", True),
    "Ambrettolide (Givaudan)":        ("iterji.com", f"{IT}/aroma-chemicals/ambrettolide-givaudan.html", True),
    "Ethylene Brassylate":            ("iterji.com", f"{IT}/aroma-chemicals/ethylene-brassylate.html", True),
    "Exaltolide Total (Firmenich)":   ("iterji.com", f"{IT}/aroma-chemicals/exaltolide-total-firmenich.html", True),
    "Cashmeran IFF":                  ("iterji.com", f"{IT}/aroma-chemicals/cashmeran-iff.html", True),
    "Sandalore (Givaudan)":           ("iterji.com", f"{IT}/aroma-chemicals/sandalore-givaudan.html", True),
    "Methyl Alpha Ionone Iso":        ("iterji.com", f"{IT}/aroma-chemicals/methyl-alpha-ionone-iso.html", True),
    "Irone Alpha":                    ("iterji.com", f"{IT}/aroma-chemicals/alpha-irone.html", True),
    "Benzyl Salicylate":              ("iterji.com", f"{IT}/aroma-chemicals/benzyl-salicylate.html", True),
    "Linalool":                       ("iterji.com", f"{IT}/aroma-chemicals/linalool.html", True),
    "Neroli Accord":                  ("iterji.com", f"{IT}/aroma-accords/neroli.html", True),
    "Petitgrain Paraguay":            ("iterji.com", f"{IT}/natural-ingredients/petitgrain-paraguay.html", True),
    "Pink Peppercorn":                ("iterji.com", f"{IT}/natural-ingredients/pink-peppercorn.html", True),
    "Cedarwood Virginia":             ("iterji.com", f"{IT}/natural-ingredients/virginian-cedarwood.html", True),
    "Bergamot Accord":                ("iterji.com", f"{IT}/aroma-accords/bergamot.html", True),
    "Bergamot Essential Oil":         ("iterji.com", f"{IT}/natural-ingredients/bergamot-essential-oil.html", True),
    "DPG (Dipropylene Glycol)":       ("iterji.com", f"{IT}/other-ingredients/dipropylene-glycol-dpg.html", True),
    "Alcohol 190 proof SDA 40B":      ("iterji.com", f"{IT}/other-ingredients/alcohol-190-proof-sda-40b.html", True),
    # ---- NOT on iterji (2026-07) — sourced elsewhere ----
    "Globalide (Habanolide)":         ("Fraterworks (intl)", "https://fraterworks.com/", False),
    "Helvetolide":                    ("Fraterworks (intl)", "https://fraterworks.com/products/helvetolide", False),
    "Javanol":                        ("Aromology (intl)", "https://aromology.com/products/javanol-super-givaudan", False),
    "Vetyveryle Acetate":             ("Perfumer's Apprentice (intl)", "https://shop.perfumersapprentice.com/", False),
    "Floralozone":                    ("Perfumer's Apprentice (intl)", "https://shop.perfumersapprentice.com/", False),
    "Safraleine":                     ("Perfumer's Apprentice (intl)", "https://shop.perfumersapprentice.com/", False),
    "Sandal Spicatum Australia":      ("Jozoor / mygezor.com (KSA)", "https://mygezor.com/", False),
    "Orange Blossom Tunisia Absolute":("Jozoor / mygezor.com (KSA)", "https://mygezor.com/", False),
}
NOT_ON_ITERJI = {m for m, (_, _, ok) in SUPPLIER.items() if not ok}
...
D10_MATS = ["Pink Peppercorn", "Floralozone", "Irone Alpha", "Javanol", "Safraleine"]
...
def put(ws, addr, value=None, font=F_VALUE, fill=None, fmt=None, align=None,
    ...
    if comment:
        c.comment = Comment(comment, "ASMR Suite")
    return c
...
def build_formula_master(wb):
    ...
    title_block(ws, 9, "ASMR — MASTER FORMULA",
                "Concentrate = 100% by weight.  D10 = weighed as a 10% dilution in DPG — the % shown is the weight of the dilution.")

    merge(ws, "A4:B4")
    put(ws, "A4", "Include optional materials (Irone Alpha, Javanol):", font=F_LABEL_B)
    ...
    opt_comment = {
        "Irone Alpha":
            "OPTIONAL — the ONE precious luxury in the heart (orris/iris butter).\n\n"
            "Expected result if INCLUDED: a buttery, powdery, suede-like violet-orris "
            "depth that reads as 'expensive' and haute-parfumerie. It rounds the heart "
            "and makes the drydown feel refined.\n\n"
            "If you LEAVE IT OUT: Methyl Alpha Ionone still carries the powdery-violet "
            "direction — the heart is simply a touch less deep. Set the toggle in C4 to "
            "NO and the workbook renormalises everything else to 100% automatically.",
        "Javanol":
            "OPTIONAL — a radiance booster in the sandalwood accord.\n\n"
            "Expected result if INCLUDED: a big lift of creamy, milky, glowing "
            "sandalwood — more 'aura' and a modern sandal signature that projects softly.\n\n"
            "If you LEAVE IT OUT: natural Sandalwood EO + Sandalore still give a rounded, "
            "creamy sandalwood — just with less radiant push. Toggle C4 = NO renormalises "
            "automatically.",
    }
    dup_comment = {
        "Sandal Spicatum Australia":
            "SANDALWOOD ACCORD (1 of 3): this natural EO is the anchor. Sandalore adds "
            "smooth volume; Javanol (optional) adds radiance. Overlapping on purpose for "
            "a full, natural-leaning sandalwood.",
        "Sandalore (Givaudan)":
            "SANDALWOOD ACCORD (2 of 3): overlaps Javanol (both creamy sandalwood "
            "synthetics). Intentional layering. If you ever simplify, drop Javanol "
            "(optional) first — keep this + the natural EO.",
        "Globalide (Habanolide)":
            "MUSK ACCORD: this and Exaltolide Total are both macrocyclic-lactone musks "
            "(very close smell). Kept together for a rounder, longer skin-musk trail — "
            "this is the effect you asked for ('still here after I leave').",
        "Exaltolide Total (Firmenich)":
            "MUSK ACCORD: overlaps Globalide (both macrocyclic-lactone musks). If you "
            "want to cut cost/complexity later, these two are the pair to review — but "
            "together they build the lingering trail.",
    }
    ...
    merge(ws, "C36:D36")
    put(ws, "C36", "optional materials are controlled only by the C4 toggle", font=F_MUTED)
    put(ws, "B37", "  OVERLAPS another  ", font=F_MUTED, fill=PatternFill("solid", start_color="DCE3E8"),
        align="center", border=B_ALL)
    merge(ws, "C37:G37")
    put(ws, "C37", "intentional accord layering — do not remove unless you choose a simpler formula", font=F_MUTED)
    merge(ws, "A38:H38")
    put(ws, "A38",
...
def build_batch_calculator(wb):
    ...
    title_block(ws, 8, "ASMR — BATCH CALCULATOR",
    ...
    input_cell(ws, "C4", "Eau de Parfum")
    input_cell(ws, "C5", 50, fmt=FMT_G)
    input_cell(ws, "C6", "ml")
    input_cell(ws, "C7", 0.20, fmt=FMT_PCT1,
               comment="Enter as a fraction (0.20 = 20%). EDP default is 20% for a wearable long-lasting version. "
    ...
    put(ws, "D8",
        ...
        font=F_MUTED)
    merge(ws, "D9:H9")
    put(ws, "D9", "optionals: Irone Alpha, Javanol — set on Formula Master", font=F_MUTED)
    ...
    band(ws, 73, 1, 8, "WORKED VERIFICATION - 50 ml EDP @ 20%, density 0.85 (fixed example)",
    ...
    ver = [
        ...
        (75, "Concentrate = 42.5 g x 20%", "=50*0.85*0.20", "expect about 8.5 g"),
        (76, "Alcohol = total minus concentrate", "=50*0.85-50*0.85*0.20", "expect about 34.0 g"),
    ]
    ...
    ws.oddFooter.center.text = "ASMR — Batch Sheet"
...
def build_dilution_prep(wb):
    ...
    for i, mat in enumerate(D10_MATS):
        ...
        put(ws, f"H{r}", f'="ASMR · "&A{r}&" · 10% in DPG · "&IF(F{r}="","(date)",TEXT(F{r},"yyyy-mm-dd"))',
...
def build_inventory(wb):
    ...
    widths(ws, {"A": 30, "B": 9, "C": 6, "D": 7, "E": 9, "F": 11, "G": 11, "H": 11,
                "I": 9, "J": 10, "K": 11, "L": 10, "M": 8, "N": 20, "O": 15, "P": 12,
                "Q": 4, "R": 4})
    ws.column_dimensions["Q"].hidden = True
    ws.column_dimensions["R"].hidden = True
    title_block(ws, 16, "INVENTORY",
                "On hand = purchases − usage from the Production Log. Prices & links verified on iterji.com (2026-07).",
                sub_cols=5)
    merge(ws, "F2:G2")
    put(ws, "F2", "Conc / batch (g):", font=F_KPI_LBL, align="right")
    put(ws, "H2", "=50*0.85*0.20", font=F_GOLD, fmt=FMT_G, align="center",
        comment="Low-stock alert basis: concentrate grams in one default EDP batch "
                "(50 ml @ 20%, density 0.85). Red rows = on hand below TWO such batches.")
    merge(ws, "I2:J2")
    put(ws, "I2", "Alcohol / batch (g):", font=F_KPI_LBL, align="right")
    put(ws, "K2", "=50*0.85*0.80", font=F_GOLD, fmt=FMT_G, align="center")

    header_cells(ws, 3, ["Material", "Pack", "Unit", "Packs", "Density g/ml", "Grams bought",
                         "Price/pack SAR", "Paid SAR", "SAR/g", "Used (g)", "On hand (g)",
                         "Alert < (g)", "Status", "Supplier", "Product link", "50 ml bottles"])
    ...
    fill_unused = PatternFill("solid", start_color="E4D3D0")   # dusty rose = not in the perfume
    unused_comment = {
        "Bergamot Essential Oil":
            "NOT USED IN THE PERFUME. You bought this, but the ASMR concentrate uses "
            "Bergamot ACCORD instead. Real bergamot EO is PHOTOTOXIC (bergapten) and can burn "
            "skin in sunlight, so it is kept out of anything worn on skin. Reserve it for "
            "scenting fabric/room only, or leave it unopened. Usage is tracked manually (0).",
    }
    for i, (mat, qty, unit, price, dens, note, assumed) in enumerate(INV_ROWS):
        r = INV_FIRST + i
        put(ws, f"A{r}", mat, fill=fill_unused if mat in unused_comment else None,
            border=B_ALL, comment=unused_comment.get(mat))
        ...
        if mat == "Bergamot Essential Oil":
            put(ws, f"J{r}", 0, font=F_INPUT, fill=FILL_INPUT, fmt=FMT_G, align="center",
                border=B_INPUT, unlocked=True, comment="Fabric use only — track manually.")
            put(ws, f"L{r}", 0, fmt=FMT_G, align="center", border=B_ALL)
        elif mat == "DPG (Dipropylene Glycol)":
        ...
        # supplier + clickable product link (col N, O)
        label, url, on_iterji = SUPPLIER.get(mat, ("—", "", True))
        if on_iterji:
            put(ws, f"N{r}", label, font=F_MUTED, align="center", border=B_ALL)
        else:
            put(ws, f"N{r}", label, font=Font(name=ARIAL, size=9, bold=True, color=RED_TX),
                fill=PatternFill("solid", start_color="F7E0C6"), align="center", border=B_ALL,
                comment=f"NOT SOLD ON iterji.com (checked 2026-07). Suggested source: {label}. "
                        f"See the Sourcing note and the Scale-Up Plan tab.")
        lc = ws[f"O{r}"]
        if url:
            lc.value = "open ↗"
            lc.hyperlink = url
            lc.font = Font(name=ARIAL, size=9, underline="single", color="1155CC")
        else:
            lc.value = ""
            lc.font = F_MUTED
        ...
        put(ws, f"P{r}", bottles, font=F_LABEL_B, fmt=FMT_INT, align="center", border=B_ALL)
        # hidden low-stock name join (cols Q, R)
        put(ws, f"Q{r}", f'=IF(K{r}<L{r},A{r},"")', font=F_MUTED)
        if r == INV_FIRST:
            put(ws, f"R{r}", f"=Q{r}", font=F_MUTED)
        else:
            put(ws, f"R{r}", f'=IF(Q{r}="",R{r - 1},IF(R{r - 1}="",Q{r},R{r - 1}&",  "&Q{r}))',
                font=F_MUTED)
    ...
    # sourcing legend + note about the off-iterji items
    sr = INV_LAST + 3
    put(ws, f"A{sr}", "  Not on iterji  ", font=Font(name=ARIAL, size=9, bold=True, color=RED_TX),
        fill=PatternFill("solid", start_color="F7E0C6"), align="center", border=B_ALL)
    merge(ws, f"B{sr}:P{sr}")
    put(ws, f"B{sr}",
        "8 ingredients are NOT sold by iterji.com (checked 2026-07): Globalide/Habanolide, Helvetolide, "
        "Javanol, Vetyveryle Acetate, Floralozone, Safraleine (captive synthetics), plus Sandal Spicatum "
        "Australia and Orange Blossom Tunisia Absolute. Their Supplier cell is highlighted and the link points "
        "to a suggested source.", font=F_MUTED, wrap=True)
    ws.row_dimensions[sr].height = 30
    note2 = [
        "The six synthetics (Givaudan/Firmenich/Symrise/IFF captives) have no Saudi retail source — buy from "
        "international specialists that ship to KSA: Fraterworks, Aromology, Perfumer's Apprentice, World of Aromas.",
        "The two naturals (Australian sandalwood, orange-blossom absolute) can often be found at Saudi essential-oil "
        "shops: Jozoor / mygezor.com, Brooq (Unaizah) brooqksa.com, or عالم الكيماويات chemicalsmed.com — verify grade before buying.",
        "Everything else is on iterji.com; click ‘open ↗’ in the Product link column. Prices re-checked 2026-07: "
        "Ambroxan and Alpha Irone rose notably, Virginian Cedarwood slightly.",
    ]
    for i, line in enumerate(note2):
        rr = sr + 1 + i
        merge(ws, f"A{rr}:P{rr}")
        put(ws, f"A{rr}", "•  " + line, font=F_MUTED, wrap=True)
        ws.row_dimensions[rr].height = 26

    ws.conditional_formatting.add(
        f"A{INV_FIRST}:P{INV_LAST}",
    ...
    name("Inv_PackQty", f"Inventory!$B${INV_FIRST}:$B${INV_LAST}")
    name("Inv_PackUnit", f"Inventory!$C${INV_FIRST}:$C${INV_LAST}")
    name("Inv_Dens", f"Inventory!$E${INV_FIRST}:$E${INV_LAST}")
    name("Inv_PricePack", f"Inventory!$G${INV_FIRST}:$G${INV_LAST}")
    ...
    name("Inv_LowNames", f"Inventory!$Q${INV_FIRST}:$Q${INV_LAST}")
    name("Inv_LowJoined", f"Inventory!$R${INV_LAST}")
...
def build_production_log(wb):
    ...
    ws["B4"].comment = Comment("Suggested ID: ASMR-YYYYMMDD-01", "ASMR Suite")
    ...
    ws["J5"].comment = Comment("Pre-filled as start + default days. Type a date over it to override.", "ASMR Suite")
...
def build_cost_analysis(wb):
    ...
    rows = [
        ...
        (5, "International shipping (SAR)", 351, FMT_SAR, True, "allocated proportionally below"),
...
def build_dashboard(wb):
    ...
    merge(ws, "B2:G2")
    put(ws, "B2", "A U R A · 3 7", font=Font(name=ARIAL, size=24, bold=True, color=CHARCOAL),
        align="center")
    ws.row_dimensions[2].height = 34
    merge(ws, "B3:G3")
    put(ws, "B3", "a signature luxury skin scent · Extrait — Mist — Cream · presence that stays",
    ...
    merge(ws, "B25:G25")
    put(ws, "B25", "ASMR personal production suite · verify against current IFRA standards before any commercial use",
...
def build_process_reference(wb):
    ...
    sections = [
        ("GOLDEN RULES", [
            ...
            "Label every container: ASMR · content · strength · date. Store everything dark and cool.",
        ]),
        ("EAU DE PARFUM 20% - MASTER BATCH METHOD (the standard)", [
            ...
            "Full masterclass: see the document 'ASMR Perfume - Master Method'.",
            "STEP 1 · PREP D10 DILUTIONS (a day ahead). Five ingredients are weighed as a 10%-in-DPG dilution: "
            "Pink Peppercorn, Floralozone, Safraleine, and (if kept) Irone Alpha + Javanol. For each: 1.0 g neat + 9.0 g DPG = 10 g. See the Dilution Prep tab.",
            "STEP 2 · SET THE MASTER BATCH. On the Batch Calculator choose Eau de Parfum, 250 ml, 20% "
            "(= 212.5 g juice: 42.5 g concentrate + 170 g alcohol - your full 5-bottle run). Print the Batch Sheet.",
            ...
            "before it meets alcohol. Amberxan crystals must be fully dissolved (liquid clear) before Step 5.",
            ...
            "Swirl once a day the first week. At 20% the EDP still benefits from full maceration - do not rush.",
            ...
            "25.5 g per 30 ml, 59.5 g per 70 ml, 85 g per 100 ml). Label each: ASMR - EDP 20% - batch ID - date.",
            "STEP 10 - LOG the ONE master batch in the Production Log (concentrate 42.5 g, alcohol 170 g) and set it to "
...
def build_safety(wb):
    ...
    notes = [
        ("CRITICAL", "Bergamot Essential Oil is PHOTOTOXIC. Skin products use the Bergamot Accord ONLY. "
                     "The EO is for fabric scenting, or at most 0.4% of a finished skin product."),
        ...
        ("RULE", "Use only ONE amber material — Amberxan (Ambroxan). Do not add a second ambroxan-type material."),
...
def build_yield_capacity(wb):
    ...
    rows = [
        (4, "Concentrate %", 0.20, FMT_PCT1, "20% = EDP default. Raise to 28% only for Extrait."),
...
def build_inventory_bottles(wb):
    ws = wb.create_sheet("Inventory Bottles")
    ws.sheet_view.showGridLines = False
    ws.sheet_properties.tabColor = GOLD
    widths(ws, {
        "A": 29, "B": 8, "C": 6, "D": 8, "E": 10, "F": 9, "G": 11, "H": 12,
        "I": 10, "J": 11, "K": 8, "L": 8, "M": 8, "N": 8, "O": 8, "P": 8,
        "Q": 8, "R": 8, "S": 8, "T": 8, "U": 8, "V": 8, "W": 11, "X": 19, "Y": 12
    })
    title_block(ws, 25, "INVENTORY & BOTTLES",
                "One live view of stock, bottle capacity by size, and which material increases output if you buy more.",
                sub_cols=25)

    sizes = [10, 20, 30, 50, 70, 100]
    current_cols = ["K", "L", "M", "N", "O", "P"]
    projected_cols = ["Q", "R", "S", "T", "U", "V"]
    first = 7
    last = first + len(INV_ROWS) - 1

    merge(ws, "A3:A5")
    put(ws, "A3", "Bottle size", font=F_KPI_LBL, align="center", border=B_ALL)
    for i, size in enumerate(sizes):
        col = chr(ord("B") + i)
        put(ws, f"{col}3", size, font=F_LABEL_B, fill=FILL_IVORY_DK, fmt=FMT_INT, align="center", border=B_ALL)
        put(ws, f"{col}4", f"=MIN({current_cols[i]}{first}:{current_cols[i]}{last})",
            font=F_KPI, fill=FILL_GOLD_LT, fmt=FMT_INT, align="center", border=B_ALL)
        put(ws, f"{col}5", f"=MIN({projected_cols[i]}{first}:{projected_cols[i]}{last})",
            font=F_KPI, fill=FILL_INPUT, fmt=FMT_INT, align="center", border=B_ALL)
    put(ws, "H3", "Current", font=F_KPI_LBL, align="center", border=B_ALL)
    put(ws, "H4", "Projected", font=F_KPI_LBL, align="center", border=B_ALL)
    merge(ws, "I3:Y5")
    put(ws, "I3",
        'Type trial purchase quantities in "Add packs". Projected bottles update immediately. '
        'Gold rows are materials where +1 pack can raise the 50 ml output; red rows are the current bottleneck.',
        font=F_MUTED, wrap=True)

    header_cells(ws, 6, ["Material", "Pack", "Unit", "Packs", "Add packs", "Density", "On hand g",
                         "Projected g", "Active %", "Use / g conc",
                         "Cur 10", "Cur 20", "Cur 30", "Cur 50", "Cur 70", "Cur 100",
                         "Proj 10", "Proj 20", "Proj 30", "Proj 50", "Proj 70", "Proj 100",
                         "+50 if +1 pack", "Priority", "Product"])

    def cap_body(r, stock, size_ref):
        mat = f"$A{r}"
        return (
            f'IF({mat}="","",'
            f'IF({mat}="DPG (Dipropylene Glycol)",'
            f'IFERROR(FLOOR((MAX({stock},0)/(D10ActivePct*0.9))/({size_ref}*YC_Density*YC_ConcPct),1),""),'
            f'IF({mat}="Alcohol 190 proof SDA 40B",'
            f'IFERROR(FLOOR(MAX({stock},0)/({size_ref}*YC_Density*(1-YC_ConcPct)),1),""),'
            f'IFERROR(FLOOR((MAX({stock},0)/(INDEX(FM_Active,MATCH({mat},FM_Mat,0))*'
            f'IF(INDEX(FM_D10,MATCH({mat},FM_Mat,0))="D10",0.1,1)))/'
            f'({size_ref}*YC_Density*YC_ConcPct),1),""))))'
        )

    f_link = Font(name=ARIAL, size=9, underline="single", color="1155CC")
    for i, row in enumerate(INV_ROWS):
        invr = INV_FIRST + i
        r = first + i
        mat = row[0]
        _, link, _ = SUPPLIER.get(mat, ("", "", True))
        put(ws, f"A{r}", f"='Inventory'!A{invr}", border=B_ALL)
        put(ws, f"B{r}", f"='Inventory'!B{invr}", fmt=FMT_INT, align="center", border=B_ALL)
        put(ws, f"C{r}", f"='Inventory'!C{invr}", font=F_MUTED, align="center", border=B_ALL)
        put(ws, f"D{r}", f"='Inventory'!D{invr}", fmt=FMT_INT, align="center", border=B_ALL)
        put(ws, f"E{r}", 0, font=F_INPUT, fill=FILL_INPUT, fmt=FMT_INT, align="center",
            border=B_INPUT, unlocked=True)
        put(ws, f"F{r}", f"='Inventory'!E{invr}", fmt="0.00", align="center", border=B_ALL)
        put(ws, f"G{r}", f"='Inventory'!K{invr}", fmt=FMT_G, align="center", border=B_ALL)
        put(ws, f"H{r}", f'=G{r}+E{r}*IF(C{r}="ml",B{r}*F{r},B{r})',
            font=F_LABEL_B, fmt=FMT_G, align="center", border=B_ALL)
        put(ws, f"I{r}",
            f'=IF(A{r}="DPG (Dipropylene Glycol)",D10ActivePct,IFERROR(INDEX(FM_Active,MATCH(A{r},FM_Mat,0)),""))',
            fmt=FMT_PCT2, align="center", border=B_ALL)
        put(ws, f"J{r}",
            f'=IF(A{r}="DPG (Dipropylene Glycol)",D10ActivePct*0.9,'
            f'IF(A{r}="Alcohol 190 proof SDA 40B","",'
            f'IFERROR(I{r}*IF(INDEX(FM_D10,MATCH(A{r},FM_Mat,0))="D10",0.1,1),"")))',
            fmt=FMT_G3, align="center", border=B_ALL)
        for col, size in zip(current_cols, sizes):
            put(ws, f"{col}{r}", "=" + cap_body(r, f"$G{r}", str(size)),
                fmt=FMT_INT, align="center", border=B_ALL)
        for col, size in zip(projected_cols, sizes):
            put(ws, f"{col}{r}", "=" + cap_body(r, f"$H{r}", str(size)),
                font=F_LABEL_B, fmt=FMT_INT, align="center", border=B_ALL)
        one_pack_stock = f'($G{r}+IF($C{r}="ml",$B{r}*$F{r},$B{r}))'
        cap_plus_one = cap_body(r, one_pack_stock, "50")
        args = [cap_plus_one]
        if r > first:
            args.append(f"MIN($N${first}:$N${r - 1})")
        if r < last:
            args.append(f"MIN($N${r + 1}:$N${last})")
        put(ws, f"W{r}", f'=IF($N{r}="","",MAX(0,MIN({",".join(args)})-$E$4))',
            font=F_LABEL_B, fmt=FMT_INT, align="center", border=B_ALL)
        put(ws, f"X{r}",
            f'=IF(AND(ISNUMBER(W{r}),W{r}>0),"BUY MORE - raises output",'
            f'IF(AND(ISNUMBER(N{r}),N{r}=$E$4),"CURRENT BOTTLENECK",'
            f'IF(AND(ISNUMBER(N{r}),N{r}<=$E$4+2),"NEXT LIMIT","")))',
            font=F_MUTED, align="center", border=B_ALL)
        lc = put(ws, f"Y{r}", "open" if link else "", font=f_link if link else F_MUTED, align="center", border=B_ALL)
        if link:
            lc.hyperlink = link

    ws.conditional_formatting.add(f"A{first}:Y{last}",
        FormulaRule(formula=[f"AND(ISNUMBER($N{first}),$N{first}=$E$4)"], fill=PatternFill("solid", start_color=RED_FILL)))
    ws.conditional_formatting.add(f"A{first}:Y{last}",
        FormulaRule(formula=[f"AND(ISNUMBER($W{first}),$W{first}>0)"], fill=PatternFill("solid", start_color=GOLD_LT)))
    dv_add = DataValidation(type="whole", operator="between", formula1="0", formula2="999")
    ws.add_data_validation(dv_add)
    dv_add.add(f"E{first}:E{last}")
    ws.freeze_panes = "A7"
    protect(ws)
...
def build_scale_up_plan(wb):
    ...
    rows = [
        ...
        (5, "Concentrate %", 0.20, FMT_PCT1, "20% EDP default. Raise to 28% only for Extrait."),
    ...
    kpis = [
        ...
        ("G", "EXTRA PACKS", "=SUM($H$19:$H$53)", FMT_INT),
        ("H", "BUY COST (SAR)", "=SUM($I$19:$I$53)", FMT_SAR),
    ...
    inv_c = f"Inventory!$C${INV_FIRST}:$C${INV_LAST}"
    inv_b = f"Inventory!$B${INV_FIRST}:$B${INV_LAST}"
    inv_e = f"Inventory!$E${INV_FIRST}:$E${INV_LAST}"
    inv_g = f"Inventory!$G${INV_FIRST}:$G${INV_LAST}"
    stock = ('IF($C$8="On hand",INDEX(Inv_OnHand,MATCH($A{r},Inv_Mat,0)),'
             'INDEX(Inv_GramsBought,MATCH($A{r},Inv_Mat,0)))')
    pack_g = (f'IF(INDEX({inv_c},MATCH($A{{r}},Inv_Mat,0))="ml",'
              f'INDEX({inv_b},MATCH($A{{r}},Inv_Mat,0))*INDEX({inv_e},MATCH($A{{r}},Inv_Mat,0)),'
              f'INDEX({inv_b},MATCH($A{{r}},Inv_Mat,0)))')
    pack_price = f'INDEX({inv_g},MATCH($A{{r}},Inv_Mat,0))'
    for i in range(len(FM_ROWS)):
        r = 19 + i
        fmrow = FM_FIRST + i
        put(ws, f"A{r}", f"='Formula Master'!$B${fmrow}", border=B_ALL)
...
def build_pricing(wb):
    ...
    ws.sheet_view.showGridLines = False
    ws.sheet_properties.tabColor = GOLD
    widths(ws, {"A": 22, "B": 13, "C": 13, "D": 12, "E": 13, "F": 11, "G": 15,
                "H": 11, "I": 15, "J": 13, "K": 12})
    title_block(ws, 9, "PRICING & SALES",
                "What to charge for the extra bottles. Juice cost is live from Cost Analysis; "
                "you set packaging and margin. Eau de Parfum pricing default.", sub_cols=11)

    # --- 1 · assumptions ---
    band(ws, 3, 1, 11, "1 · PRICING ASSUMPTIONS  (champagne cells are yours to set)")
    rows = [
        (4, "Concentrate %", 0.20, FMT_PCT1, "20% EDP - matches this product"),
    ...
    # --- 2 · price per bottle ---
    band(ws, 10, 1, 11, "2 · COST → RETAIL PER BOTTLE")
    header_cells(ws, 11, ["Bottle size (ml)", "Juice cost", "Packaging", "Labour+OH",
                          "TOTAL COST", "Cost/ml", "Retail ex-VAT", "Retail/ml",
                          "Retail inc-VAT", "Profit / bottle", "Gross margin"])
    juice = ('$A{r}*PR_Density*PR_ConcPct*ConcCostPerGLoaded'
             '+$A{r}*PR_Density*(1-PR_ConcPct)*AlcCostPerG')
    pack_default = {10: 12, 20: 18, 30: 30, 50: 40, 70: 48, 100: 55}
    for i, s in enumerate([10, 20, 30, 50, 70, 100]):
        r = 12 + i
        put(ws, f"A{r}", s, font=F_LABEL_B, fill=FILL_IVORY_DK, fmt=FMT_INT, align="center", border=B_ALL)
        ...
        put(ws, f"E{r}", f"=B{r}+C{r}+D{r}", font=F_LABEL_B, fill=FILL_GOLD_LT, fmt=FMT_SAR,
            align="center", border=B_ALL)
        put(ws, f"F{r}", f'=IF(A{r}=0,"",E{r}/A{r})', fmt=FMT_SAR, align="center", border=B_ALL)
        put(ws, f"G{r}", f"=E{r}*PR_Mult", font=F_KPI, fill=FILL_IVORY, fmt=FMT_SAR,
            align="center", border=B_ALL)
        put(ws, f"H{r}", f'=IF(A{r}=0,"",G{r}/A{r})', fmt=FMT_SAR, align="center", border=B_ALL)
        put(ws, f"I{r}", f"=G{r}*(1+PR_Vat)", fmt=FMT_SAR, align="center", border=B_ALL)
        put(ws, f"J{r}", f"=G{r}-E{r}", fmt=FMT_SAR, align="center", border=B_ALL)
        put(ws, f"K{r}", f'=IF(G{r}=0,"",(G{r}-E{r})/G{r})', fmt=FMT_PCT1, align="center", border=B_ALL)

    # --- 3 · multiple reference (50 ml) ---
    band(ws, 19, 1, 11, "3 · WHAT DIFFERENT MULTIPLES GIVE  (50 ml, ex-VAT)")
    header_cells(ws, 20, ["Multiple", "×2 (friends)", "×2.5", "×3 (fair indie)", "×4 (luxury)",
                          "", "", "", "", "", ""])
    merge(ws, "F20:K20")
    put(ws, "F20", "market: a 50 ml niche extrait often retails 400–900+ SAR", font=F_HDR,
        fill=FILL_HDR, align="center", border=B_ALL)
    put(ws, "A21", "Retail (SAR)", font=F_LABEL_B, border=B_ALL)
    put(ws, "B21", "=$E$15*2", fmt=FMT_SAR, align="center", border=B_ALL)
    put(ws, "C21", "=$E$15*2.5", fmt=FMT_SAR, align="center", border=B_ALL)
    put(ws, "D21", "=$E$15*3", fmt=FMT_SAR, align="center", border=B_ALL)
    put(ws, "E21", "=$E$15*4", fmt=FMT_SAR, align="center", border=B_ALL)
    merge(ws, "F21:K21")
    put(ws, "F21", '="Your total cost for a 50 ml = "&TEXT($E$15,"0.00")&" SAR, cost/ml = "&TEXT($F$15,"0.00")&" SAR, retail/ml = "&TEXT($H$15,"0.00")&" SAR"',
        font=F_MUTED, border=B_ALL)

    # --- 4 · batch economics ---
    band(ws, 23, 1, 11, "4 · BATCH ECONOMICS  (50 ml bottles at your target price)")
    econ = [
        (24, "Bottles to sell", 4, FMT_INT, "you can make ~5 × 50 ml — keep some, sell the rest (see Yield & Capacity)"),
        (25, "Revenue ex-VAT (SAR)", "=PR_NSell*$G$15", FMT_SAR, "bottles × retail ex-VAT"),
        (26, "Your cost (SAR)", "=PR_NSell*$E$15", FMT_SAR, "bottles × total cost"),
        (27, "Gross profit (SAR)", "=PR_NSell*($G$15-$E$15)", FMT_SAR, "before VAT, fees, selling costs"),
        (28, "Break-even price / bottle", "=$E$15", FMT_SAR, "sell below this and you lose money"),
    ]
    ...
    # --- 5 · before you sell ---
    band(ws, 30, 1, 11, "5 · BEFORE YOU SELL — PLEASE READ", fill=PatternFill("solid", start_color=RED_TX),
    ...
    name("PR_NSell", "'Pricing & Sales'!$C$24")
    protect(ws)
...
def build_selling_strategy(wb):
    ...
    ws.sheet_view.showGridLines = False
    ws.sheet_properties.tabColor = GOLD
    widths(ws, {"A": 20, "B": 12, "C": 11, "D": 13, "E": 11, "F": 13,
                "G": 13, "H": 13, "I": 11, "J": 16, "K": 22})
    title_block(ws, 9, "SELLING STRATEGY",
                "Proposed launch pricing and channel plan connected to live cost, yield, VAT, "
                "and scale-up assumptions.", sub_cols=11)

    band(ws, 3, 1, 11, "1 · STRATEGY INPUTS")
    rows = [
    ...
    band(ws, 10, 1, 11, "2 · PROPOSED PRICE LADDER")
    header_cells(ws, 11, ["Bottle", "Cost", "Cost/ml", "Base retail", "Retail/ml",
                          "Launch price", "Inc VAT", "Profit", "Margin",
                          "Positioning", "Recommendation"])
    for r, size, pr_row, positioning, recommendation in [
        (12, 10, 12, "Sample", "Use for trial sets"),
        (13, 20, 13, "Travel", "Small paid tester"),
        (14, 30, 14, "Discovery", "Use as entry bottle"),
        (15, 50, 15, "Hero SKU", "Lead with this"),
        (16, 70, 16, "Premium", "Limited upsell"),
        (17, 100, 17, "Gift / collector", "Limited upsell"),
    ]:
        ...
        put(ws, f"D{r}", f"='Pricing & Sales'!$G${pr_row}", fmt=FMT_SAR, align="center", border=B_ALL)
        put(ws, f"E{r}", f"='Pricing & Sales'!$H${pr_row}", fmt=FMT_SAR, align="center", border=B_ALL)
        put(ws, f"F{r}", f"=CEILING(D{r}*(1+$C$7),10)", font=F_KPI, fill=FILL_GOLD_LT,
            fmt=FMT_SAR, align="center", border=B_ALL)
        put(ws, f"G{r}", f"=F{r}*(1+$C$8)", fmt=FMT_SAR, align="center", border=B_ALL)
        put(ws, f"H{r}", f"=F{r}-B{r}", fmt=FMT_SAR, align="center", border=B_ALL)
        put(ws, f"I{r}", f'=IF(F{r}=0,"",H{r}/F{r})', fmt=FMT_PCT1, align="center", border=B_ALL)
        put(ws, f"J{r}", positioning, font=F_MUTED, align="center", border=B_ALL)
        put(ws, f"K{r}", recommendation, font=F_MUTED, align="center", border=B_ALL)

    band(ws, 20, 1, 11, "3 · CHANNEL PLAN  (50 ml hero bottle)")
    header_cells(ws, 21, ["Channel", "Units", "Price ex-VAT", "Commission", "Net revenue",
                          "Unit cost", "Gross profit", "Action", "Notes", "", ""])
    channels = [
        (22, "Private circle", "=MAX($C$4-B23-B24,0)", "=$F$15*(1-$C$7)", 0.0,
         "Start with warm demand; keep it personal."),
        (23, "Samples / testers", "=MIN($C$5,$C$4)", 0.0, 0.0,
         "Only for feedback and proof, not giveaways without intent."),
        (24, "Retail / boutique", "=ROUND(MAX($C$4-B23,0)*0.25,0)", "=$F$15", "=$C$6",
    ...
    for r, channel, units, price, commission, note in channels:
        ...
        put(ws, f"D{r}", commission, fmt=FMT_PCT1, align="center", border=B_ALL)
        put(ws, f"E{r}", f"=B{r}*C{r}*(1-D{r})", fmt=FMT_SAR, align="center", border=B_ALL)
        put(ws, f"F{r}", "='Pricing & Sales'!$E$15", fmt=FMT_SAR, align="center", border=B_ALL)
        put(ws, f"G{r}", f"=E{r}-B{r}*F{r}", fmt=FMT_SAR, align="center", border=B_ALL)
        put(ws, f"H{r}", "Sell first" if r == 18 else ("Track feedback" if r == 19 else "Defer"),
            font=F_MUTED, align="center", border=B_ALL)
        put(ws, f"I{r}", note, font=F_MUTED, wrap=True, border=B_ALL)

    band(ws, 27, 1, 11, "4 · GO / NO-GO CHECK")
    checks = [
    ...
    name("StrategyHeroPrice", "'Selling Strategy'!$F$15")
    protect(ws)
...
def main():
    ...
    order = ["Dashboard", "Formula Master", "Batch Calculator", "Dilution Prep (D10)",
             "Inventory", "Inventory Bottles", "Production Log", "Maceration Tracker", "Cost Analysis",
    ...
    wb.properties.title = "ASMR Production Suite"
    wb.properties.creator = "ASMR"
