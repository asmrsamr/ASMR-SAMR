...
VERSION_RE = re.compile(r"^ASMR (\d{1,4})$")
...
def main() -> None:
    ...
    # 4 · report
    print(f"  version → ASMR {tag}  ({len(deliverables)} files + version info)")
    print(f"  local   → {HERE / f'ASMR {tag}'}")
    if drive_ok:
        print(f"  drive   → {DRIVE_ROOT / f'ASMR {tag}'}")
