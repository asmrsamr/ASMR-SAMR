...
FM_ROWS = [
    ...
    ("BASE", "Vanillin Crystals", 0.070, False, False, "warm vanilla — the addiction itself"),
    ...
    ("BASE", "Coumarin", 0.045, False, False, "tonka warmth — almond-hay softness"),
...
def build_process_reference(wb):
    ...
    sections = [
        ...
        ("EAU DE PARFUM 20% - MASTER BATCH METHOD (the standard)", [
            ...
            "25.5 g per 30 ml, 59.5 g per 70 ml, 85 g per 100 ml). Label each: SAMR - EDP 20% - batch ID - date.",
...
def build_safety(wb):
    ...
    notes = [
        ...
        ("CRITICAL", "Damascone Beta is IFRA-RESTRICTED and extremely powerful. Weigh it ONLY as the 10% D10 "
                     "dilution and never raise it above the 0.3% line (about 0.006% of the finished EDP)."),
        ("RULE", "Use only ONE amber material — Ambroxan. Do not add a second ambroxan-type material."),
        ("RULE", "Coumarin at 4.5% of the concentrate is about 0.90% of a 20% EDP - 60% of the IFRA cat-4 ceiling "
                 "(1.5%). Never raise coumarin above 5.0% or the concentration above 20% without re-running "
                 "compliance_check.py against the current IFRA standard."),
...
def build_inventory_bottles(wb):
    ...
    for i, row in enumerate(INV_ROWS):
        ...
        put(ws, f"X{r}",
            ...
            font=F_MUTED, align="center", border=B_ALL)
        lc = put(ws, f"Y{r}", "open", font=f_link, align="center", border=B_ALL)
        lc.hyperlink = link
...
def build_pricing(wb):
    ...
    header_cells(ws, 20, ["Multiple", "×2 (friends)", "×2.5", "×3 (fair indie)", "×4 (luxury)",
                          "", "", "", "", "", ""])
    merge(ws, "F20:K20")
    put(ws, "F20", "market: a 50 ml niche EDP often retails 300–750+ SAR", font=F_HDR,
    ...
    econ = [
        (24, "Bottles to sell", 4, FMT_INT, "you can make ~7 × 50 ml — keep some, sell the rest (see Yield & Capacity)"),
...
def build_selling_strategy(wb):
    ...
    pricing_rows = [(12, 10, 12), (13, 20, 13), (14, 30, 14), (15, 50, 15), (16, 70, 16), (17, 100, 17)]
    for r, size, pr_row in pricing_rows:
        ...
        positioning = {10: "Sample", 20: "Travel", 30: "Discovery", 50: "Hero SKU", 70: "Premium", 100: "Gift / collector"}[size]
        recommendation = {10: "Use for trial sets", 20: "Small paid tester", 30: "Use as entry bottle", 50: "Lead with this", 70: "Limited upsell", 100: "Limited upsell"}[size]
...
def build_the_scent(wb):
    ...
    merge(ws, "B3:G3")
    put(ws, "B3", "Eau de Parfum 20% · Gourmand Amber Floral · a warm-skin scent",
        font=F_SUB, align="center")
    for col in range(2, 8):
        ws.cell(row=4, column=col).fill = FILL_GOLD
    ws.row_dimensions[4].height = 3
    ...
    para(r, "SAMR is a private EDP built around warmth on skin: soft fruit at the opening, white "
    ...
    para(r, "Eau de Parfum at 20% — apply low and close: pulse of the wrists, the hollow of the "
