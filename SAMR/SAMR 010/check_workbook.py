...
JUICE_50 = 50 * 0.85 * 0.20 * CONC_LOADED + 50 * 0.85 * 0.80 * ALC_LOADED
...
def bottles(size_ml):
    return int(LIMIT_G // (size_ml * 0.85 * 0.20))
...
def main():
    ...
    # ---- scenario 1: defaults (EDP, 50 ml, 20%, 0.85) ----------------
    print("\n[1] Defaults - Formula Master sums + EDP 50 ml @ 20%")
    recalc(TEST)
    wb = values(TEST)
    check("input % sum = 100%", wb[FM]["C33"].value, 1.0, 1e-9)
    check("active % sum = 100% (optionals ON)", wb[FM]["F33"].value, 1.0, 1e-9)
    check("default format is EDP", 1.0 if wb[BC]["C4"].value == "Eau de Parfum" else 0.0, 1.0)
    check("total batch mass = 42.5 g", wb[BC]["C21"].value, 42.5)
    check("concentrate = 8.5 g (20%)", wb[BC]["C23"].value, 8.5)
    check("per-material sum = concentrate (±0.01)", wb[BC]["E54"].value, 8.5, 0.01)
    check("alcohol = 34.0 g", wb[BC]["D58"].value, 34.0)
    ...
    check("weigh sum still = concentrate", wb[BC]["E54"].value, 8.5, 0.01)
    ...
    check(f"10 ml EDP bottles = {bottles(10)}", wb[YC]["F16"].value, bottles(10))
    check(f"30 ml EDP bottles = {bottles(30)}", wb[YC]["F18"].value, bottles(30))
    check(f"50 ml EDP bottles = {bottles(50)}", wb[YC]["F19"].value, bottles(50))
    check(f"100 ml EDP bottles = {bottles(100)}", wb[YC]["F21"].value, bottles(100))
    ...
    recalc(TEST)
    wb = values(TEST)
    check(f"50 ml juice cost ≈ {JUICE_50:.2f} SAR", wb[PR]["B15"].value, JUICE_50, 0.2)
