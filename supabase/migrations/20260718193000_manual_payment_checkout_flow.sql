-- Manual-payment launch checkout flow.
-- This keeps WhatsApp as the checkout path while making payment preference and
-- launch order statuses first-class admin fields.
...
alter table public.orders
  ...
  check (
    ...
    or payment_method in ('bank_transfer', 'stc_transfer', 'cod_meetup', 'paypal', 'manual', 'other')
  );
...
alter table public.orders
  ...
  check (
    status in (
      ...
      'pending_payment', 'payment_sent', 'paid',
...
begin
  ...
  if v_payment_method is not null
     and v_payment_method not in ('bank_transfer', 'stc_transfer', 'cod_meetup', 'paypal', 'manual', 'other') then
  ...
  insert into public.orders (
    ...
    customer_city, customer_address, source_route, payment_method, type, status,
    ...
    nullif(trim(p_source_route), ''), coalesce(v_payment_method, 'bank_transfer'),
    p_type, 'pending_payment', 'web', v_subtotal, v_vat, v_total, 'SAR',
    nullif(trim(p_note), '')
...
comment on function public.submit_storefront_order(
  text, text, text, text, text, text, text, text, text, text, jsonb
) is 'Creates a WhatsApp-first storefront order with payment preference, server-priced items, VAT totals, structured customer delivery context, and inventory availability checks.';

create or replace view public.admin_business_overview
with (security_invoker = true)
as
select
  (select coalesce(sum(total), 0) from public.orders where status in ('paid', 'preparing', 'ready', 'shipped', 'delivered')) as total_sales,
  (select count(*) from public.orders) as total_orders,
  (select coalesce(avg(total), 0) from public.orders where status <> 'cancelled') as average_order_value,
  (select count(*) from public.orders where status in ('pending_payment', 'payment_sent', 'awaiting_confirmation', 'confirmed')) as pending_orders,
